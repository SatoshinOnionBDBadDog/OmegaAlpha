...
public class Gateway {
	...
	private ThumbnailStorePrx getThumbService() {
		try {
			...
			// handling should be done by OMERO dev
		}
		return null;
	}
	 ...
	 */
	public boolean login(LoginCredentials credentials) throws Exception {
		// read login file
		// parse
	...
	public List<ImageData> getImages() throws Exception {
		...
		try {
			...
			po.exp(omero.rtypes
			        .rlong(getAdminService().getEventContext().userId));
	 ...
	 * @throws Exception
	 */
	public List<DatasetData> getDatasets(List<Long> ids) throws Exception {
		List<DatasetData> datasets = new ArrayList<DatasetData>();
		ParametersI po = new ParametersI();
		po.exp(omero.rtypes.rlong(getAdminService().getEventContext().userId));
	...
	public RenderingEnginePrx loadRenderingControl(long pixelsID)
	        throws Exception {
		try {
			RenderingEnginePrx service = (RenderingEnginePrx) reServices
			        .get(pixelsID);
			if (service != null)
				return service;
	...
	public int getByteWidht(long pixelsID) {
		RawPixelsStorePrx service = null;
		try {
			service = entryEncrypted.createRawPixelsStore();
			service.setPixelsId(pixelsID, false);
			return service.getByteWidth();
		} catch (ServerError e) {
			// nothing we can do: return the "default"
			return 2;
		} finally {
			try {
				service.close();
	...
	public List<IObject> loadPlaneInfo(long pixelsID, int z, int t, int channel)
	        throws Exception {
		// isSessionAlive();
		...
		try {
			List<IObject> info = service.findAllByQuery(sb.toString(), param);
			return info;
		} catch (Exception e) {
